...
public class JsLexerTests
{
    ...
    [TestMethod]
    public void Unicode_space_separators_are_skipped_without_line_terminator()
    {
        var spaces = "\u00A0\u1680\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200A\u202F\u205F\u3000";

        var tokens = Tokens($"a{spaces}b");

        tokens.Select(t => t.Kind).Should().Equal(
            JsTokenKind.Identifier,
            JsTokenKind.Identifier,
            JsTokenKind.EndOfFile);
        tokens[1].Lexeme.Should().Be("b");
        tokens[1].PrecededByLineTerminator.Should().BeFalse();
    }
